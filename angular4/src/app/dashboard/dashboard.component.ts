...
export class DashboardComponent {
  ...
  constructor(http: Http) { //Injetando a dependência do Http no constructor. Sempre que o component for carregado o Http será instanciado
    ...
    http.get(url) //Envia um GET para a url passada
      .map(res => res.json()) //recebe a resposta em uma callback e mapeia pra json()
      .subscribe(({credit = 0, debt = 0}) => { //Operador destructoring | Pega o objeto da resposta e desestrutura nas variáveis passadas
        this.credit = credit
        this.debt = debt
